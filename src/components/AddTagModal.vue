<template>
	<BaseModal title="Add Tag" @close="$emit('close')">
		...
		<template #actions>
			...
			<button class="btn btn-accent" @click="submit" :disabled="error">Add</button>
		</template>
	</BaseModal>
</template>
...
const error = computed(() => !tagInput.value.trim() || tagInput.value.trim().toLowerCase() === "uncategorized")
