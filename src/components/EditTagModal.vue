<template>
	<BaseModal title="Edit Tag" @close="$emit('close')">
		...
		<template #actions>
			...
			<button class="btn btn-accent" @click="submit" :disabled="saving || disableError">
				{{ saving ? "Saving…" : "Save" }}
			</button>
		</template>
	</BaseModal>
...
const error = ref("")
const disableError = computed(() => !nameInput.value.trim() || nameInput.value.trim().toLowerCase() === "uncategorized")
