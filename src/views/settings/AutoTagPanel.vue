  ...
  <section class="settings-section">
    ...
    <div v-if="rules.length > 0" class="rules-list">
      <div v-for="rule in rules" :key="rule.id" class="rule-row">
        ...
        <span
          class="rule-tag"
          :style="{ background: tagStore.getColor(rule.tagName), color: '#fff' }"
        >
          {{ rule.tagName }}
        </span>
    ...
    <div v-if="rules.length > 0" class="preview-section">
      ...
      <div v-if="previewFiles.length > 0" class="preview-list">
        ...
        <div v-if="previewFiles.length > 50" class="preview-more">
          ...and {{ previewFiles.length - 50 }} more files
        </div>
      </div>
      ...
      <div v-if="applySuccess" class="success-message">
        Applied tags to {{ lastApplyCount }} files
      </div>
    </div>
  </section>
</template>
...
const patternPlaceholder = computed(() => {
  switch (newRule.value.type) {
    case 'filename': return 'e.g. kick, snare, pad'
    case 'regex': return 'e.g. ^(kick|snare)_\\d+'
    case 'extension': return 'e.g. .wav, .mp3'
    default: return ''
  }
})
...
function matchCount(rule: AutoTagRule): number {
  return library.files.filter(f =>
    fileMatchesRule(f, rule) && !f.tags.includes(rule.tagName)
  ).length
}
