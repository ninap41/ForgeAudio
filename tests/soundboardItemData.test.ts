...
describe('SoundboardItemDataModal', () => {
  ...
  it('shows uniqueId when present', () => {
    setupTestData()
    const store = useSoundboardStore()
    const sb = store.allSoundboards.find(s => s.id === sbId)!
    const item = sb.items.find(i => i.id === itemId)!
    // uniqueId is auto-generated by addItem
    const wrapper = createWrapper()
    const rows = wrapper.findAll('.data-row dd')
    // Row 11: Unique ID
    expect(rows[11].text()).toBe(item.uniqueId ?? '—')
    expect(wrapper.find('.data-monospace').exists()).toBe(true)
  })
  ...
  it('shows parent soundboard name', () => {
    ...
    const rows = wrapper.findAll('.data-row dd')
    // Row 12: Soundboard
    expect(rows[12].text()).toBe('Test Board')
  })
  ...
  it('shows "Partial: Enabled" when item.partial is true', () => {
    ...
    const wrapper = createWrapper()
    const rows = wrapper.findAll('.data-row dd')
    // Row 13: Partial Playback
    expect(rows[13].text()).toBe('Enabled')
  })
  ...
  it('shows "Partial: Disabled" when item.partial is false', () => {
    ...
    const wrapper = createWrapper()
    const rows = wrapper.findAll('.data-row dd')
    expect(rows[13].text()).toBe('Disabled')
  })
  ...
  it('shows range values when set', () => {
    ...
    const rows = wrapper.findAll('.data-row dd')
    // Row 15: Range
    expect(rows[15].text()).toBe('0:00 – 0:02')
  })
  ...
  it('shows item duration formatted', () => {
    ...
    const wrapper = createWrapper()
    const rows = wrapper.findAll('.data-row dd')
    // Row 16: Item Duration
    expect(rows[16].text()).toBe('0:02')
  })
